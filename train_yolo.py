...
def get_dataset(dataset, args):
    if dataset.lower() == 'voc':
        train_dataset = VOCLike(root=r'D:\QMDownload\VOCdevkit', splits=[(2028, 'trainval')])
        val_dataset = VOCLike(root=r'D:\QMDownload\VOCdevkit', splits=[(2028, 'test')])
...
def get_dataloader(net, train_dataset, val_dataset, data_shape, batch_size, num_workers, args):
    ...
    if args.no_random_shape:
        train_loader = gluon.data.DataLoader(
            ...
            batch_size, True, batchify_fn=batchify_fn, last_batch='rollover', num_workers=num_workers)     #数据增强
    else:
